...
from wgpy_backends.webgpu.shader_util import header, native_scalar_type_for_type, native_pixel_type_for_internal_format
from wgpy_backends.webgpu.kernel_common import GenericResolveResult, make_input_uniform, make_output_uniform, InParam, OutParam, UniformDefinition, parse_in_params, parse_out_params, parse_uniforms, resolve_generic_type
...
def get_input_key(name, out_name, ndim, elementwise, dtype, texture_shape: WebGPUArrayTextureShape):
    ...
    """
    return (name, ndim, elementwise, dtype, texture_shape.dim, texture_shape.wgsl_dtype)

def make_output_key(name, ndim, dtype, texture_shape: WebGPUArrayTextureShape):
    """
    Obtain the key that is the branching factor for kernel generation
    """
    return (name, ndim, dtype, texture_shape.dim, texture_shape.wgsl_dtype)

def make_input_def(param: InParam, out_name, ndim, dtype, texture_shape: WebGPUArrayTextureShape):
