...
from wgpy_backends.webgpu.kernel_common import GenericResolveResult, make_input_reduction_uniform, make_input_uniform, make_output_uniform, InParam, OutParam, parse_in_params, parse_out_params, parse_uniforms, resolve_generic_type
...
def make_input_def(param: InParam, out_name, ndim, dtype, texture_shape: WebGPUArrayTextureShape, binding_index: int, axis: Tuple[int, ...]):
...
def make_output_def(param: OutParam, ndim, dtype, texture_shape: WebGPUArrayTextureShape, binding_index: int):
    ...
    for d in range(ndim):
        meta_defs.append(WebGPUMetaBufferItem(f"_{name}_shape_{d}", 'i32'))
    meta_defs.append(WebGPUMetaBufferItem(f"_out_ind_size", 'i32')) # cupy's _ind.size()
...
def make_reduction_loop(axis: Tuple[int, ...], input_shape: Tuple[int, ...], out_texture_shape: WebGPUArrayTextureShape, reduce_type: Optional[str], identity: str):
...
class ReductionKernel:
    ...
    def __init__(self, in_params: str, out_params: str, map_expr: str, reduce_expr: str, post_map_expr: str, identity: str, name: str, reduce_type: Optional[str]=None, uniforms: Union[str, List[WebGPUMetaBufferItem]]='', preamble: str='', no_return: bool=False, return_tuple: bool=False) -> None:
    ...
    def _generate_kernel_source(self, in_array_impls: List[ndarray], out_array_impl: ndarray, generic_resolve_result: GenericResolveResult, axis: Tuple[int, ...], input_shape: Tuple[int, ...]):
        ...
        meta_def_all.append(WebGPUMetaBufferItem(f"_in_ind_size", 'i32')) # cupy's _in_ind.size()
...
"""
        ...
        for k, ary in zip(self.parsed_in_params, in_array_impls):
            # return meta_defs, func_def, inner_head, variable_binding_source
            meta_defs, func_def, inner_head, binding_source_part = make_input_def(k, self.parsed_out_param.name, ary.ndim, ary.dtype, ary.buffer.texture_shape, next_binding_index, axis)
            meta_def_all.extend(meta_defs)
...
"""
    ...
    def __call__(self, *arrays, axis: Optional[Union[int, Tuple[int, ...]]]=None, keepdims: bool=False, uniforms: Optional[dict]=None) -> ndarray:
        ...
        if kernel_name not in added_kernels:
            source, meta_defs, binding_types = self._generate_kernel_source(in_array_impls, out_array_impl_squeeze, generic_resolve_result, n_axis, input_shape)
