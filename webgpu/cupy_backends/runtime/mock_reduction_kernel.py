...
def crossent_fwd(reduction_kernel, args, out=None, axis=None, keepdims=False, stream=None):
...
def mock_reduction_kernel(reduction_kernel, args, out=None, axis=None, keepdims=False, stream=None):
    if reduction_kernel.name == 'crossent_fwd':
        return crossent_fwd(reduction_kernel, args, out=out, axis=axis, keepdims=keepdims, stream=stream)
    else:
