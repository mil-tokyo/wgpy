...
import numpy as np
from wgpy_backends.webgl.texture import WebGL2RenderingContext, WebGLArrayTextureShape, get_default_texture_shape
from wgpy_backends.webgl.shader_util import header, native_pixel_type_for_internal_format
...
class WebGLBuffer:
    ...
    dtype: np.dtype # logical type (may be different from physical representation in WebGL)
    texture_shape: WebGLArrayTextureShape

    _comm_buf: Optional[np.ndarray] = None
    next_id = 1

    def __init__(self, size: int, dtype: np.dtype, texture_shape: Optional[WebGLArrayTextureShape]=None) -> None:
        self.size = size
        self.dtype = dtype
        ...
        if pooled_buffer_id is not None:
            ...
            performance_metrics['webgl.buffer.buffer_size'] += self.texture_shape.element_count * texture_type_to_element_itemsize[self.texture_shape.type]
            performance_metrics['webgl.buffer.buffer_count_max'] = max(performance_metrics['webgl.buffer.buffer_count_max'], performance_metrics['webgl.buffer.buffer_count'])
            performance_metrics['webgl.buffer.buffer_size_max'] = max(performance_metrics['webgl.buffer.buffer_size_max'], performance_metrics['webgl.buffer.buffer_size'])
    ...
    def _get_comm_buf(self, byte_size: int) -> np.ndarray:
        if WebGLBuffer._comm_buf is None or WebGLBuffer._comm_buf.size < byte_size:
            WebGLBuffer._comm_buf = np.empty((max(byte_size, 1024 * 1024),), dtype=np.uint8)
    ...
    def set_data(self, array: np.ndarray):
        if self.texture_shape.type == WebGL2RenderingContext.HALF_FLOAT:
            ...
            buf = self._get_comm_buf(np.dtype(np.uint16).itemsize * self.texture_shape.element_count)
            packed = buf.view(np.float16)
            packed[:array.size] = array_f16
            size = self.texture_shape.element_count
            dtype = np.uint16
        else:
            dtype = {WebGL2RenderingContext.FLOAT: np.float32, WebGL2RenderingContext.INT: np.int32, WebGL2RenderingContext.UNSIGNED_BYTE: np.uint8}[self.texture_shape.type]
            buf = self._get_comm_buf(np.dtype(dtype).itemsize * self.texture_shape.element_count)
        ...
        # physical size
        performance_metrics['webgl.buffer.write_size'] += size * np.dtype(dtype).itemsize
        # logical size
        if array.size <= 1:
    ...
    def get_data(self) -> np.ndarray:
        ...
        if copied is not None:
            return copied._get_data_internal(self.texture_shape.elements_per_pixel == 1, self.dtype)
        else:
    ...
    def _get_data_internal(self, extract_r_from_rgba: bool, original_dtype: np.dtype):
        ...
        if self.texture_shape.type == WebGL2RenderingContext.HALF_FLOAT:
            buf = self._get_comm_buf(np.dtype(np.uint16).itemsize * self.texture_shape.element_count)
            get_platform().getData(self.buffer_id, get_dtype_js_ctor_type(np.uint16), self.texture_shape.element_count)
            performance_metrics['webgl.buffer.read_size'] += self.texture_shape.element_count * np.dtype(np.uint16).itemsize
            if self.size <= 1:
            ...
            dtype = {WebGL2RenderingContext.FLOAT: np.float32, WebGL2RenderingContext.INT: np.int32, WebGL2RenderingContext.UNSIGNED_BYTE: np.uint8}[self.texture_shape.type]
            buf = self._get_comm_buf(np.dtype(dtype).itemsize * self.texture_shape.element_count)
            get_platform().getData(self.buffer_id, get_dtype_js_ctor_type(dtype), self.texture_shape.element_count)
            performance_metrics['webgl.buffer.read_size'] += self.texture_shape.element_count * np.dtype(np.uint16).itemsize
            if self.size <= 1:
                performance_metrics['webgl.buffer.read_scalar_count'] += 1
    ...
    def _copy_to_rgba_if_needed(self) -> Optional['WebGLBuffer']:
        is_32bit = self.texture_shape.type in [WebGL2RenderingContext.FLOAT, WebGL2RenderingContext.INT]
        ...
        native_pixel_type_dst = native_pixel_type_for_internal_format[target_shape.internal_format]
        ...
        new_size = self.size if self.texture_shape.elements_per_pixel == 4 else self.size * 4
        target_buffer = WebGLBuffer(size=new_size, dtype=target_dtype, texture_shape=target_shape)

        kernel_source = None
        if target_shape.dim == '2D':
            kernel_name = f'copy_r_to_rgba_2d_{native_pixel_type_src}_{native_pixel_type_dst}'
