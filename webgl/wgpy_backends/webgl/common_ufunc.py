...
truediv = create_ufunc('truediv', ['BB->f', 'ii->f', 'ff->f'], 'out0 = float(in0) / float(in1)')
...
neg = create_ufunc('neg', [
    ('?->?', 'out0 = !in0'), # note: error in numpy
    ('B->B', 'out0 = (256u - in0) & 255u'),
    'i->i',
    'f->f',
], 'out0 = -in0')
abs = create_ufunc('abs', [
    ('?->?', 'out0 = in0'),
    ('B->B', 'out0 = in0'),
    'i->i',
    'f->f',
], 'out0 = abs(in0)')
invert = create_ufunc('invert', [
    ('?->?', 'out0 = !in0'),
    ('B->B', 'out0 = 255u - in0'),
    'i->i',
], 'out0 = ~in0')
